...
const sortedEvents = computed(() => {
  const events = [...(timelineEvents.value || [])]
    ...
    .sort((a, b) => {
      const dateA = new Date(a.updated_date || a.posted_date)
      const dateB = new Date(b.updated_date || b.posted_date)
    ...
    <div class="container mx-auto px-4 py-8 max-w-2xl">
      <div class="flex justify-between items-center mb-8">
        ...
        <div class="flex items-center space-x-2">
          ...
          <button
            class="px-2 py-1 text-sm font-medium text-foreground bg-background border border-border rounded-lg hover:bg-muted focus:outline-none focus:ring-2 focus:ring-primary"
            @click="toggleOrder"
          >
            <Icon
              :name="isAscending ? 'uil:sort-amount-down' : 'uil:sort-amount-up'"
